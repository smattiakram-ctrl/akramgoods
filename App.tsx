...
const App: React.FC = () => {
  const [user, setUser] = useState<any | null>(db.getUser());
    ...
    const { data: { subscription } } = db.supabase.auth.onAuthStateChange((event, session) => {
      if (session?.user) {
        const userProfile = {
          ...
          picture: session.user.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${session.user.email}&background=1d4ed8&color=fff`,
        };
  ...
  const handleManualSync = async () => {
    await loadData(true);
    alert('تمت مزامنة البيانات من Supabase بنجاح ✅');
  };
  ...
  const handleLogin = async (newUser: any) => {
    const userProfile = {
      id: newUser.id,
      email: newUser.email,
      name: newUser.user_metadata?.name || newUser.user_metadata?.full_name || newUser.email.split('@')[0],
      picture: newUser.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${newUser.email}&background=1d4ed8&color=fff`,
    };
    db.saveUser(userProfile);
    setUser(userProfile);
  ...
  const handleSale = async (productId: string, qty: number, price: number) => {
    ...
    await db.saveEarnings(newEarnings);
  ...
  const inventoryValue = useMemo(() => {
    return products.reduce((sum, p) => {
      const price = parseFloat(p.price.split('/')[0]) || 0;
      return sum + (price * p.quantity);
    }, 0);
  }, [products]);

  if (isLoading && user) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><RefreshCw className="w-10 h-10 text-blue-600 animate-spin" /></div>;

  if (!user) {
    return (
      <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-center font-['Cairo'] overflow-hidden">
        ...
        <div className="relative z-10 max-w-sm">
          <div className="bg-white/5 backdrop-blur-xl p-10 rounded-[4rem] border border-white/10 mb-10 shadow-2xl">
            ...
            <h1 className="text-4xl font-black text-white mb-3 tracking-tighter">NABIL Inventory</h1>
            <p className="text-gray-400 font-medium">مخزنك سحابي بالكامل مع Supabase. تسجيل دخول سريع عبر GitHub.</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24 font-['Cairo']">
      ...
      <div className={`fixed top-0 right-0 h-full w-80 bg-white z-[70] shadow-2xl transition-all duration-500 transform ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col">
          ...
          <nav className="flex-1 space-y-4">
            <button onClick={() => { setView('HOME'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-bold transition ${view === 'HOME' ? 'bg-blue-50 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}><Home className="w-6 h-6" /> الرئيسية</button>
            <button onClick={() => { setView('SALES_LOG'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-bold transition ${view === 'SALES_LOG' ? 'bg-orange-50 text-orange-700' : 'text-gray-500 hover:bg-gray-50'}`}><History className="w-6 h-6" /> سجل المبيعات</button>
          </nav>
          <div className="pt-8 border-t space-y-4">
             <div className="bg-slate-900 p-6 rounded-[2.5rem] text-white">
                <p className="text-[10px] text-slate-400 font-black mb-2 uppercase">الحساب المتصل</p>
        ...
        <div className="flex-1 max-w-xl relative">
          ...
          <input 
            ...
            onChange={(e) => {
              ...
              if (e.target.value && (view === 'HOME' || view === 'CATEGORY_DETAIL')) setView('SEARCH');
              if (!e.target.value && view === 'SEARCH') setView('HOME');
            }}
            className="w-full pr-10 pl-4 py-3 bg-gray-100/60 border-0 rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all"
          />
        </div>
        <div className="flex items-center gap-3">
          <button onClick={handleManualSync} disabled={isSyncing} className={`p-2 bg-slate-50 rounded-xl text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition ${isSyncing ? 'animate-spin text-blue-600' : ''}`}>
             <RefreshCw className="w-5 h-5" />
          </button>
          <button onClick={() => setShowAuthModal(true)} className="w-10 h-10 rounded-xl overflow-hidden ring-2 ring-blue-50 shadow-sm hover:scale-105 active:scale-95 transition">
          ...
          <div className="space-y-10">
             ...
             {view === 'HOME' && !searchQuery && (
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <button onClick={() => setShowProductForm(true)} className="p-6 bg-green-500 text-white rounded-[2.5rem] font-black flex flex-col items-center gap-3 shadow-lg hover:bg-green-600 transition active:scale-95"><Plus className="w-8 h-8" /> إضافة سلعة</button>
                  <button onClick={() => setIsScanning(true)} className="p-6 bg-blue-500 text-white rounded-[2.5rem] font-black flex flex-col items-center gap-3 shadow-lg hover:bg-blue-600 transition active:scale-95"><Camera className="w-8 h-8" /> مسح باركود</button>
                  <button onClick={() => setShowSaleDialog(true)} className="p-6 bg-orange-500 text-white rounded-[2.5rem] font-black flex flex-col items-center gap-3 shadow-lg hover:bg-orange-600 transition active:scale-95"><ShoppingCart className="w-8 h-8" /> عملية بيع</button>
                  <button onClick={() => setShowCategoryForm(true)} className="p-6 bg-slate-800 text-white rounded-[2.5rem] font-black flex flex-col items-center gap-3 shadow-lg hover:bg-slate-900 transition active:scale-95"><LayoutGrid className="w-8 h-8" /> صنف جديد</button>
               </div>
             )}

             <div className="space-y-6">
                <h3 className="text-xl font-black text-slate-900 flex items-center justify-between">
                   {searchQuery ? `نتائج البحث (${filteredProducts.length})` : 'أصناف المخزن'}
                   {searchQuery && <button onClick={() => {setSearchQuery(''); setView('HOME');}} className="text-xs text-blue-600 font-bold">إلغاء البحث</button>}
                </h3>
                ...
                {searchQuery || view === 'SEARCH' ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                    {categories.map(c => (
                      <div key={c.id} className="group bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl transition-all cursor-pointer active:scale-95" onClick={() => { setSelectedCategoryId(c.id); setView('CATEGORY_DETAIL'); }}>
                        ...
                        <div className="p-5 text-center">
                          ...
                          <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">{products.filter(p => p.categoryId === c.id).length} سلع</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowProductForm(true)} className="bg-blue-600 text-white px-6 py-4 rounded-[1.5rem] font-black text-sm flex items-center gap-2 shadow-xl shadow-blue-100 hover:bg-blue-700 active:scale-95 transition">
      ...
      <div className="fixed bottom-0 inset-x-0 h-20 bg-white/95 backdrop-blur-md border-t flex items-center justify-around md:hidden z-50 px-4">
         ...
         <button onClick={() => setShowSaleDialog(true)} className="w-14 h-14 bg-orange-500 text-white rounded-[1.5rem] shadow-xl shadow-orange-200 flex items-center justify-center -translate-y-6 border-4 border-[#f8fafc] active:scale-90 transition transform">
